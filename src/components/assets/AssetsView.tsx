...
import { JPY_PER_CNY } from "@/lib/fx";
import {
...
function convertToDisplay(
  ...
  if (from === "JPY" && display === "CNY") return amount / JPY_PER_CNY;
...
export function AssetsView() {
  ...
  const [budgetLimits, setBudgetLimits] = useState<
    { category: string; limit: number; currency: Currency }[]
  >([]);
  ...
  const total = useMemo(() => {
    return accounts.reduce((sum, a) => {
      return sum + convertToDisplay(a.balance, a.currency, displayCurrency);
    }, 0);
    ...
    <>
      ...
      <section className="mb-6 rounded-2xl border border-stone-200 bg-gradient-to-br from-emerald-50 to-white p-5 dark:border-neutral-800 dark:from-emerald-950/40 dark:to-neutral-900">
        ...
        <p className="mt-2 text-[11px] text-stone-400">
          演示汇率 1 CNY = {JPY_PER_CNY} JPY，可后续在设置中修改
        </p>
      </section>
